...
export const Debug = {
  ...
  writeLog: function(msg)
  {
    try
    {
      if (SpreadsheetApp.getActiveSpreadsheet())
      {
        ...
        var logEntry = Utilities.formatString("[%s]  %s\n", time, msg);
        ...
        catch (e)
        {
          ...
          debugLog = Utilities.formatString("%s\n\n%s", purgeMsg, debugLog.substring(Math.floor(len/2)));
          cache.put(this.CACHE_DEBUG_LOG, debugLog, this.CACHE_DEBUG_LOG_EXPIRATION);
        }
      }
      else
  ...
  getExceptionInfo: function(e)
  {
    ...
    for (var prop in e) {
      exInfo += Utilities.formatString("    %s:  '%s'\n", prop, e[prop]);
    }
